...
class VideoRecorder(Node):
    def __init__(self):
        ...
        self.declare_parameter('timestamp_format', '%y%m%d_%H%M')  # 예: 250930_1803
        topic   = self.get_parameter('topic').value
        self.out_path = self.get_parameter('out_path').value
        use_ts = bool(self.get_parameter('use_timestamp').value)
        ts_fmt = str(self.get_parameter('timestamp_format').value)
        if use_ts:
            base, ext = os.path.splitext(self.out_path)
            stamp = datetime.now().strftime(ts_fmt)
            self.out_path = f"{base}_{stamp}{ext}"
